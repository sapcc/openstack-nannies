...
if [ "$VCENTER_CLEANUP_BIGVM_SHARES_ACTION_SIZE" != "" ]; then
    BIGVM_SHARES_ACTION_SIZE="--bigvm-shares-action-size $VCENTER_CLEANUP_BIGVM_SHARES_ACTION_SIZE"
else
    BIGVM_SHARES_ACTION_SIZE=""
fi
...
python3 /scripts/vcenter-cleanup.py $DRY_RUN $POWER_OFF $UNREGISTER $DELETE $DETACH_GHOST_PORTS $DETACH_GHOST_VOLUMES $DETACH_GHOST_LIMIT $VOL_CHECK $BIGVM_SIZE $BIGVM_SHARES_ACTION_SIZE --host $VCENTER_CLEANUP_HOST --username $VCENTER_CLEANUP_USER --password $VCENTER_CLEANUP_PASSWORD --iterations $VCENTER_CLEANUP_ITERATIONS --interval $VCENTER_CLEANUP_INTERVAL
